...
export const DEFAULT_SERVER_INSTRUCTIONS = `You are connected to the user's workspace via Portal, a deliberately minimal MCP server: commands and file transfer only.
...
- If the workspace is a WSL folder, Portal keeps the tunnel on Windows but maps the workspace to \\\\wsl.localhost\\<distro>\\... and runs default commands with wsl.exe inside that distro. Explicit shell=powershell/cmd still runs on Windows against the UNC path.
...
export class McpHttpServer {
  ...
  constructor(
    ...
    private readonly hooks?: McpActivityHooks,
    private readonly routeToken = "",
  ...
  async stop(): Promise<void> {
    ...
    // survive a bridge stop or app reload.
  ...
  private async onBody(req: http.IncomingMessage, res: http.ServerResponse, aborted: boolean, body: string): Promise<void> {
    ...
    try {
      ...
      this.respondError(res, 400, -32700, "Parse error");
      return;
    }
