// Tunnel lifecycle: ngrok + cloudflared + custom.
// Ported from the Portal extension (vscode-free).
import { execFile, spawn } from "node:child_process";
import { existsSync } from "node:fs";
import * as http from "node:http";
import * as https from "node:https";
import { promisify } from "node:util";
import type { CustomTunnelShell, ErrorAdvice, TunnelProvider } from "../shared/types";
...
export class TunnelError extends Error {
  constructor(
    message: string,
    public readonly provider: TunnelProvider,
    public readonly advice?: ErrorAdvice,
...
export async function installCloudflaredViaWinget(): Promise<void> {
  ...
  try {
    await execOut(
      "winget",
      ["install", "--id", "Cloudflare.cloudflared", "-e", "--source", "winget", "--accept-package-agreements", "--accept-source-agreements"],
      180_000,
    );
    return;
  } catch (e: any) {
...
export async function installNgrokViaWinget(): Promise<void> {
  ...
  await execOut(
    "winget",
    ["install", "--id", "Ngrok.Ngrok", "-e", "--source", "winget", "--accept-package-agreements", "--accept-source-agreements"],
    180_000,
  );
}
...
export async function startNgrok(
  ...
  const consume = (kind: "out" | "err", b: Buffer) => {
    ...
    if (/ERR_NGROK_\d+/.test(output) || /failed to start tunnel/i.test(output)) {
      ...
      rejectFail(
        new TunnelError(t("err.ngrokStartFailed", code || tail() || "unknown error"), "ngrok-reserved", matchKnownError(output), output),
      );
    }
  };
  ...
  child.once("exit", (code, signal) => {
    ...
    if (!settled) {
      ...
      rejectFail(
        new TunnelError(
          t("err.ngrokExited", `exit=${code ?? "null"}${signal ? ` signal=${signal}` : ""} ${tail()}`),
          "ngrok-reserved",
          matchKnownError(output),
          output,
        ),
      );
    }
  ...
  void waitForNgrokUrl(reservedDomain, 20_000, onLog).then(
    (url) => {
      if (!settled) {
        settled = true;
        ready = true;
...
async function waitForNgrokUrl(reservedDomain: string, timeoutMs: number, onLog: (s: string) => void): Promise<string> {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      const found = (tunnels?.tunnels ?? []).find((x: any) =>
        String(x?.public_url ?? "").endsWith(reservedDomain.replace(/^https?:\/\//i, "")),
      );
...
function customShellSpec(shell: CustomTunnelShell, cmdline: string): { file: string; args: string[]; shell?: boolean } {
  switch (shell) {
    case "powershell":
      return { file: "powershell.exe", args: ["-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", cmdline] };
    case "pwsh":
      return { file: "pwsh", args: ["-NoProfile", "-Command", cmdline] };
    case "cmd":
      return isWin ? { file: "cmd.exe", args: ["/d", "/s", "/c", cmdline] } : { file: cmdline, args: [], shell: true };
    case "bash":
      return { file: "bash", args: ["-c", cmdline] };
    default:
...
export async function startCustomTunnel(
  ...
  if (!command) {
    ...
    return { provider: "custom", publicUrl: fixedUrl, pid: 0, stop: async () => {} };
  }
  ...
  const succeedIfReady = () => {
    ...
    if (fixedUrl) {
      if (Date.now() - startedAt >= CUSTOM_DWELL_MS) {
        settled = true;
        ready = true;
        resolveReady();
      }
...
async function killProcessTree(child: TunnelChild, label: string, onLog: (s: string) => void): Promise<void> {
  ...
  try {
    child.kill();
    onLog(`[${label}] killed pid=${child.pid}`);
  } catch {
    /* ignore */
  }
}
