/** run_command: bounded foreground execution with optional terminal mirroring. */
...
export const runCommand: ToolModule = {
  name: "run_command",
  description:
    "Run a foreground command inside the workspace. Default timeout is 120 seconds. Use command+shell for shell syntax, or executable+args for injection-safe direct argv execution. Windows supports powershell, pwsh, cmd, and Git Bash (bash/sh). The result starts with structured metadata.",
  inputSchema: {
    type: "object",
    properties: {
