/**
 * File I/O for a WSL workspace when the host is on Windows.
 *
 * All reads/writes go through `wsl.exe` instead of fs (which can be unreliable
 * on \\wsl.localhost UNC paths). Do not use `bash -lc '... $1'` — a login shell
 * plus `wsl.exe --` drops positional arguments.
 */
 ...
 * are cut at traversal time (not filtered after the fact).
 */
