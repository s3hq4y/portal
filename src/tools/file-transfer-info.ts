 ...
 * examples). This is the ONLY way to read/write files through Portal.
 */
import { err, text, ToolModule } from "./types";

export const fileTransferInfo: ToolModule = {
  name: "file_transfer_info",
  description: "Return the public HTTP file-transfer base URL on the same tunnel (GET/PUT/POST /files/<token>/...). File read/write tools are intentionally not exposed by this server; use this HTTP API for all files, binary or text.",
